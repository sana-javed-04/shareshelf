...
def to_out(db: Session, tx: Transaction, pin: str | None = None) -> TransactionOut:
    ...
    active_pin = pin or getattr(tx, "pickup_pin_plain", None)
...
def request_item(
    ...
    # Duplicate check: Sirf active ya pending transactions ko block karein
    duplicate = db.scalar(
        select(Transaction).where(
            Transaction.item_id == item.id,
            Transaction.borrower_id == user.id,
            Transaction.status.in_(["Pending", "Active", "Disputed"]),
        )
    )
    if duplicate:
...
def verify_pin(
    ...
    item = db.get(Item, tx.item_id)
    if item is not None:
        curr_avail = getattr(item, "available_quantity", 1)
        new_avail = max(0, curr_avail - 1)
...
def mark_returned(
    ...
    if tx.status == "Completed":
        raise HTTPException(status.HTTP_409_CONFLICT, "This transaction is already completed.")
    ...
    if item is not None and item.listing_type == "RENT":
        item.available_quantity = getattr(item, "available_quantity", 0) + 1
        item.status = "Available"
...
def dispute_transaction(
    ...
    tx = db.get(Transaction, tx_id)
    # Owner aur Borrower dono dispute trigger kar sakein
    if tx is None or user.id not in (tx.owner_id, tx.borrower_id):
