...
def serialize(db: Session, item: Item, distance: float | None = None) -> ItemOut:
    ...
    total_qty = getattr(item, "quantity", 1) or 1
    avail_qty = getattr(item, "available_quantity", None)

    # Agar status Available hai aur available_quantity 0 ya None hai, to total_qty ke barabar lein
    if item.status == "Available" and (avail_qty is None or avail_qty == 0):
        avail_qty = total_qty
    elif avail_qty is None:
        avail_qty = total_qty
    ...
    data = {
        ...
        "security_deposit": getattr(item, "security_deposit", 0.0),
        "quantity": total_qty,
...
def list_items(
    ...
    if owner_id is not None:
        ...
        if status_filter:
            ...
            stmt = stmt.where(Item.status == "Available")
...
def create_item(
    ...
    qty = getattr(payload, "quantity", 1) or 1
    ...
    item = Item(
        ...
        security_deposit=payload.security_deposit if payload.listing_type == "RENT" else 0.0,
        quantity=qty,
...
def update_item(
    ...
    # Quantity & Available Quantity Sync Fix:
    # Agar quantity update ki gayi ho ya status Available kiya gaya ho
    new_qty = getattr(item, "quantity", 1) or 1
    if "quantity" in update_data:
        # Active transactions count karein
        active_rentals = db.scalar(
            select(func.count(Transaction.id)).where(
                Transaction.item_id == item.id,
                Transaction.status.in_(["Active", "Pending"])
            )
        ) or 0
        
        # Available units nikal kar assign karein
        item.available_quantity = max(0, new_qty - active_rentals)

    if item.status == "Available" and (getattr(item, "available_quantity", 0) <= 0):
        item.available_quantity = new_qty
